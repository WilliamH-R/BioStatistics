...
Support Vector Machines are an example of non-linear modelling, as they can model non-linear relationships between the predictors and the outcome depending on the chosen kernel. Utilizing the strengths of kernels, SVMs can model complex relationships between the variables and the outcome.

The risk of overfitting is much higher for non-linear models emphasizing the need for cross-validation and hyperparameter optimization.

This section include less explanation of the code as it has the same structure as the previous section.
...
The cross-validation objects are created.
...
The ranges of the hyperparameters are changed. Note that it does not change the model specification, but the information is saved in a new object and can be used to update the model specification in the next chunk.
...
The workflow set is created by combinding the recipes and specifications.
...
Performance across the grid search is visualized.
...
The best hyperparameters are extracted, saved and added to the workflow to finalize.
