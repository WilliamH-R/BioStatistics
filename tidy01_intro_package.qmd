...
As is customary when modeling, the dataset is split into a training- and testing set. We need to check for class imbalance in case some responses (here species) are more likely in the dataset. As seen from @fig-iris_species_count, it is not the case.
```{r}
#| label: fig-iris_species_count
