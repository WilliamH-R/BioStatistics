...
To fit and make predictions for each model use mapping functions from the `purrr` package. We also calculate the accuracy using the `yarstick` package.
```{r}
...
dimension_models %>%
  ...
  group_by(wflow_id) %>% 
  yardstick::accuracy(truth = Species,
                      estimate = .pred_class)
```