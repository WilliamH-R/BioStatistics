...
For the ENet models, two parameters are tuned: `penalty` and `mixture`. Instead of plotting against AUC of ROC, the hyperparameters are plotted against each other, and the color represents the AUC of ROC. The best AUC of ROC are the more purple colors. The plot shows that the best AUC of ROC is achieved with a penalty close to zero.
...
Again, the hyperparameter values are plotted against each other, and the colour signifies the AUC of ROC. The best AUC of ROC are the more purple colors.
