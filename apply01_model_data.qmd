...
The data is split:
```{r}
count_matrix_split <- initial_split(count_matrix, prop = 0.80)
...
`tidymodels` comes with a lot of convenience functions. One of these is `autoplot()` that can, among other things, plot the results of the grid search. The best result of each model is selected based on the accuracy. As seen, they all perform quite similarly with an accuracy of approximately $0.9$.
...
```{r}
grid_results |>
  unnest(result) |> collect_metrics(result)
...
```{r}
autoplot(grid_results, metric = "accuracy", id = "stand_log")
```

See what is done here : https://workflowsets.tidymodels.org/articles/tuning-and-comparing-models.html#tuning-the-models

Especially the end where they show how to extract the best model and use it to predict the test set.