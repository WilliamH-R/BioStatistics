...
By removing variables, the algorithm can be used for dimensionality reduction. To exemplify, the below code chunk tries to predict `mpg` given all variables using Lasso Regression. The expectation is, that some variables contain too little information about the outcome, and is excepted at higher values of $\lambda$.
...
# Model recipe, no preprocessing
lasso_recipe <- recipe(mpg ~ .,
                       data = mtcars_train)

# Combine to worflow
lasso_wflow <- workflow_set(
  preproc = list(recipe = lasso_recipe),
  models = list(OLS = OLS_spec,
                Lasso = lasso_spec)
  )
...
# Run through grid
lasso_grid_results <- lasso_wflow |> 
  workflow_map(
    seed = 1337,
    resamples = mtcars_folds,
    grid = 5,
...
# Model recipe, no preprocessing
enet_recipe <- recipe(mpg ~ .,
                      data = mtcars_train)

# Combine to worflow
enet_wflow <- workflow_set(
  preproc = list(recipe = enet_recipe),
...
####
#### ALSO SEE WHAT THE DEFAULTS ARE FOR TUNING MIXTURE
####

# Add params to workflow
enet_wflow <-
  enet_wflow |>
  option_add(id = "recipe_ENet",
