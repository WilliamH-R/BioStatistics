...
meta |> 
  ...
  labs(title = "BMI distribution",
       ...
       y = "Count")
```
The Shannon Index is then calculated. As mentioned in Chapter 1, it is a within sample metric, so which group a sample belongs to is irrelevant. The Wilcoxon Rank Sum test performed for each group and gathered:
...
Some way to correct for multiple testing is needed to avoid these high probabilities of false positives. There are several methods for correcting for multiple testing. Some common methods are the Bonferroni correction and the Benjamini-Hochberg Procedure
...
p_values_per_group |> 
  ...
  mutate(critical_value = (rank / nrow(p_values_per_group)) * alpha) |> 
  filter(p_value <= critical_value)
```
...
Starting with the highest rank, the adjusted p-value is simply the intermediary value found from the above. For the second highest rank, the adjusted p-value is the minimum of the intermediary value and the adjusted p-value of the rank above. This is repeated for all ranks. The adjusted p-values are then compared to the significance level $\alpha$, and all p-values less than the significance level are considered significant.

```{r}
p_values_per_group |> 
  arrange(p_value) |> 
  mutate(rank = row_number()) |> 
  mutate(p_value_adjusted = p_value * (nrow(p_values_per_group) / rank)) #|> 
  #mutate(p_value_adjusted = pmin(p_value_adjusted,
  #                               dplyr::lag(p_value_adjusted, default = Inf)))
```
